...
AimbotSettingsTab:AddSlider("PredictionVelocity", { Text = "Prediction Scale", Default = 0.75, Min = 0, Max = 15, Rounding = 2, Suffix = "" })
...
BotsSettingTab:AddDropdown("BotMovementStyle", { Text = "Movement Style", Values = { "Analyse", "Junius", "Ban" }, Default = 1 })
...
RunService.RenderStepped:Connect(function()
    ...
    if Toggles.AimbotToggle.Value and CurrentTarget and ValidateTarget(CurrentTarget) then
        UserInputService.MouseBehavior = Enum.MouseBehavior.LockCurrentPosition
    else
        UserInputService.MouseBehavior = Enum.MouseBehavior.Default
    end
    
    local isCrosshairVisible = Toggles.AimbotToggle.Value and not Toggles.DisableCrosshair.Value
    ...
    if myHum and myHum.Health > 0 and myRoot then
        ...
        if Toggles.EnableWalkSpeed.Value then
            ...
            -- Mode "Ban" speed diatur secara dinamis di bawah pada blok pergerakannya
        end
        ...
        if Toggles.AutoWalkToClosest.Value then
            ...
            if botTarget then
                ...
                elseif currentStyle == "Ban" then
                    ...
                    -- Deteksi jika ada player lain/target yang sedang melihat posisi kita
                    for _, p in pairs(Players:GetPlayers()) do
                        if p ~= LocalPlayer and IsAlive(p) then
                            local eRoot = p.Character:FindFirstChild("HumanoidRootPart")
                            if eRoot then
                                local toMe = (myRoot.Position - eRoot.Position).Unit
                                if eRoot.CFrame.LookVector:Dot(toMe) > 0.72 then -- Player tersebut sedang melihat kita
                                    isBeingWatched = true
                                    break
                                end
                            end
                    ...
                    -- Jika dilihat musuh: Kecepatan Max (28) & Pergerakan Zig-Zag Ekstrim
                    ...
                    -- Mengaplikasikan noise pergerakan bawaan Ban mode
                    ...
                    -- Deteksi player dekat dan lengah (tidak melihat kita) untuk ganti arah putaran (Flank)
                    ...
                    -- Peek Jump Logic: Deteksi musuh di balik tembok tapi bisa dilihat jika melompat
                    if not IsVisible(botTarget, targetRoot) then
                        if t - lastPeekJump > 0.45 then
                            local startPos = myRoot.Position + Vector3.new(0, 6.5, 0) -- Ketinggian udara simulasi melompat
                ...
                if (myRoot.Position - lastPosition).Magnitude < 0.15 then
                    ...
                    if stuckTime > 1.0 then
                        myHum:MoveTo(myRoot.Position - myRoot.CFrame.LookVector * 10)
                ...
                -- Sistem Deteksi dan Auto Jump untuk Dinding/Obstacle Pendek
                ...
                if legBlock and not headBlock then
                    myHum.Jump = true
                end
